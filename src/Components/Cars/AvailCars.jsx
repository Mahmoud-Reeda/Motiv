...
function classNamesAvailability(...classes) {
  return classes.filter(Boolean).join(" ");
}
const AvailCars = () => {
  const [availability, setAvailability] = useState("New");
          ...
          <div className="relative mt-2 ">
            <Listbox.Button className="min-w-[150px] !rounded-[50px] relative w-full cursor-default rounded-md bg-white py-1.5 pl-3 pr-10 text-left text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 sm:text-sm sm:leading-6">
              <span className="flex items-center">
                <span className="ml-3 block truncate">{availability}</span>
              </span>
                  ...
                  {({ availability, active }) => (
                    <>
                      <div className="flex items-center">
                        <span
                          className={classNamesAvailability(
                            availability ? "font-semibold" : "font-normal",
                            "ml-3 block truncate"
                          )}
                        >
                          New
